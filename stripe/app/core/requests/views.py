...
class SubmitRequestView(APIView):
    """Manually (re)submit the KYC onboarding request for the signed-in
    company. Onboarding normally auto-starts on connect; this stays as an
    ...
    def post(self, request: Request) -> Response:
        request_id = PermytClient().start_onboarding(
            request.user, request.data.get("description")
        )
        return Response({"request_id": request_id})


class OnboardingStateView(APIView):
    """Return the current onboarding state for the signed-in company.

    The dashboard polls this on load so a reload mid-flow (or after the
    account was created) reflects the real state — onboarding auto-starts on

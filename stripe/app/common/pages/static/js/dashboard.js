/* Stripe-KYC onboarding dashboard: live stage track + provenance reveal,
   driven by Log rows the broker callback upserts as the request advances. */
...
(function () {
  ...
  // ── Stage track ──────────────────────────────────────────────────
  const STAGE_ORDER = ["sent", "approval", "approved", "fetching", "pushing", "done"];
  const STATUS_TO_STAGE = {
    submitted: "sent", queued: "sent", analyzing: "sent",
    awaiting: "approval",
    processing: "approved",
    fetching: "fetching",
    pushing_stripe: "pushing",
    account_created: "done", data_received: "done",
  };
  ...
  function setStage(name, failed) {
    ...
    stageSteps.forEach((step) => {
      ...
      else if (i === idx) step.classList.add(name === "done" ? "done" : "active");
  ...
  function showResult(data) {
    ...
    acctIdEl.textContent = data.stripe_account_id || "";
    const req = data.requirements || {};
    const due = (req.currently_due || []).length;
    requirementsEl.innerHTML = due === 0
      ? '<span class="req-ok"><i data-lucide="check"></i> None outstanding — currently_due is empty</span>'
      : '<span class="req-pending">' + due + " outstanding: " + escapeHtml((req.currently_due || []).join(", ")) + "</span>";
    if (data.stub) {
      requirementsEl.innerHTML += '<div class="req-note">stub mode — set STRIPE_SECRET_KEY for a live account</div>';
    }
  ...
  function applyRow(fields) {
    ...
    if (d.stripe_account_id) showResult(d);
    if (failed) {
      setLive("We couldn't complete onboarding — " + (d.reason || d.error || d.note || "please try again."), true);
      if (retryBtn) retryBtn.hidden = false;
    } else if (d.stripe_account_id || stage === "done") {
      setLive(null);
      if (retryBtn) retryBtn.hidden = true;
    }
  }

  // ── Retry: re-fire the onboarding request after a failure ──────────
  function resetForRetry() {
    ...
    if (retryBtn) retryBtn.hidden = true;
    setLive("Verifying &amp; creating your account…", false);
  }
  ...
  // Onboarding auto-starts on connect; reflect whatever state already exists

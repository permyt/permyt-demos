...
class TokenMiddleware:
    """
    Allows to get the user from the request token.
    """
    ...
    def __call__(self, request):
        ...
        try:
            authorization = request.headers.get("authorization") or "Token <none>"
            _, token = authorization.split(" ")
            request._token_user = User.objects.get(auth_token__key=token)
        except Exception:  # pylint: disable=broad-except
            request._token_user = None

...
class PermytClient(BasePermytClient):
    ...
    def _handle_completion(self, record, services: list[Any]) -> None:
        """Record completion + call provider endpoints (if any) on the same row."""
        if not services:
            record(
                action="data_received",
                data={"status": "data_received", "note": "no services"},
            )
            return

        record(
            action="calling_services",
            data={"status": "calling_services", "num_services": len(services)},
        )
        try:
            responses = self.call_services(services)
        except Exception as exc:  # pylint: disable=broad-exception-caught
            record(
                action="call_services_error",
        ...
        record(
            ...
            data={
                ...
                "responses": [r if isinstance(r, dict) else str(r) for r in responses],
            },
        )

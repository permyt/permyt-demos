...
One endpoint handles all broker callbacks. `PermytInboundView` receives signed POST requests and delegates to `client.handle_inbound()`, which verifies the signature, validates the nonce, and routes by action type (`user_connect`, `request_status`, etc.). The view itself is minimal — the SDK does the heavy lifting.

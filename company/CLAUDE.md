...
This file provides guidance to Claude Code (claude.ai/code) when working with the Company Agent demo provider.

## Project Overview

**company-agent/** is a PERMYT demo provider called **Company Agent**. It is a Django app on port 9017 (production: `company.permyt.io`) that subclasses `permyt.PermytClient` from `permyt-api-python` and implements the Provider + Connect endpoints of the PERMYT protocol.

Each user has a fixed citizen profile (full_name, birthdate, address, country, vat, phone, email, tax_id). The provider exposes a **static** scope catalogue:

- 8 direct read scopes (one per profile field).
- 3 privacy-preserving predicate scopes that take an input and return a single boolean: `is_older.check(min_age)`, `is_resident_of.check(country_code)`, `vat_matches.check(value)`.

Predicates leak one bit instead of the underlying field, so the broker's AI scope selector and the user's mobile-app consent UI both prefer them over raw reads.
...
python manage.py runserver 9017            # run dev server
```
  ...
  app/
    ...
    core/
      ...
      requests/         # PermytClient, RequestToken, Nonce, webhook + per-scope endpoints
        scopes/         # CompanyAgentScopes + ScopeDescriptor catalogue
...
| `CompanyAgentScopes`     | `app/core/requests/scopes/utils.py`       | Static scope catalogue — validation + execute, backed by `SCOPES`       |
...
PERMYT webhook → PermytClient.process_request()
  → CompanyAgentScopes.validate_params()   # locked-input enforcement (predicates fail-closed)
  → CompanyAgentScopes.execute(user, ref)  # read profile col / compute predicate
```

### Scope catalogue (`CompanyAgentScopes`)
...
- Scopes are descriptor-driven (`SCOPES` tuple in `catalogue.py`). Add one entry per new scope; do not subclass `CompanyAgentScopes`.
...
### This Project's Role: Company Agent Provider
...
- **Request cycle (step 7)**: `store_token()` receives `TokenRequestData`, canonicalizes the locked scope grant via `CompanyAgentScopes.validate_locked()`, persists `RequestToken`. SDK issues encrypted JWT.
- **Request cycle (step 10)**: `process_request()` validates JWT via `get_token_metadata()`, enforces locked-input matching per scope (predicates fail-closed if `locked={}`), and dispatches to `CompanyAgentScopes.execute()` which runs the descriptor's executor against the user's profile.

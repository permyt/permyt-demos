# Bank — PERMYT Demo Provider

A working Django app that demonstrates how to build a **bank Provider** using the [`permyt`](https://github.com/LeopardLabs/permyt-api-python) Python SDK.
...
Bank implements the **Provider** and **Connect** roles for PERMYT access requests, with a fake bank account per user — a single IBAN, a current balance, a base currency, and a small history of mock movements seeded on first connect.

It exposes three scopes:

1. **`balance.read`** (no inputs) → `{ amount, currency, iban }`
2. **`movements.list`** (no inputs) → `{ movements: [ … last 20 transactions … ] }`
3. **`payment.send`** → `{ payment: { id, status, account, value, currency, name, description, timestamp } }`
   - `account` — beneficiary account / IBAN **(locked)**
   - `value` — amount to transfer **(locked)**
   - `currency` — ISO 4217 code **(locked)**
   - `name` — beneficiary display name (free, set by requester at call time)
   - `description` — payment description (free, set by requester at call time)

The "where the money goes, how much, in what currency" trio is broker-locked — the user approves exact values on their mobile app and tampering between approval and execution is rejected. Labelling fields (name, description) are free-form so the requester can fill them in at call time without re-prompting the user.
...
| `process_user_connect()` | Handles the Connect callback — creates/links users, seeds the account |
...
`SCOPES` is a tuple of `ScopeDescriptor` records. Each entry maps a scope reference (e.g. `payment.send`) to a name, description, optional input serializer, and an executor callable. `BankScopes` (in `scopes/utils.py`) implements the same public surface the SDK expects — `validate_params`, `validate_locked`, `execute`, `get_endpoint` — but reads from the static catalogue instead of the database.

To add a new scope, append one descriptor to `SCOPES` and re-run `python manage.py update_scope`.

### Locked-input enforcement

The financially-significant bank inputs (`account`, `value`, `currency`) are **locked at the broker** — the user approves exact values and they are canonicalized into the request token. At call time, `validate_params` re-runs the input through the same serializer with `locked_data=` set, raising `InvalidInputError` if a locked field doesn't match the approved value. A scope that declares any locked fields and arrives with an empty `locked` dict is rejected outright (fail-closed). Free-form labelling fields (`name`, `description`) live on the same serializer but outside `Meta.locked_fields`, so the requester can set them at call time without going through the user.
...
### Profile + movements API — [`app/core/requests/views.py`](app/core/requests/views.py)

`ProfileView` (`GET/PUT /rest/profile/`) and `MovementsView` (`GET /rest/movements/`) are dashboard-only endpoints — they are **not** part of the PERMYT protocol. The dashboard's WebSocket-driven refresh fetches them after a `payment.send` execution pushes a `balance_changed` event.

### Connect flow — [`app/common/pages/views.py`](app/common/pages/views.py)

The Connect flow is a general-purpose mechanism for linking a user's PERMYT identity to an action in the provider. The pattern: generate a connect token with `client.generate_connect_token()`, store a record of what that token is for, and render the QR payload. When the user scans the code, the broker calls back to the inbound webhook with the user's identity. `process_user_connect()` then authenticates the user and calls `seed_profile()` to populate the IBAN, starting balance, and a small history of mock movements.

When the user revokes the connection from their PERMYT app, the broker fires a mirror `user_disconnect` callback. `process_user_disconnect()` is idempotent. Regular users have no non-PERMYT login path, so the `User` row is deleted outright — cascade wipes the IBAN, balance, movements, and login tokens. Privileged users (staff / superuser / account manager) keep their account; only `permyt_user_id` is nulled and the login tokens are dropped so they can still reach the admin paths.
...
python manage.py runserver 9013
```
...
- **Name**: "Bank"
- **Callback URL**: `http://localhost:9013/rest/permyt/inbound` (production: `https://bank.permyt.io/rest/permyt/inbound`)
...
│   │   ├── bank/           # Movement model + seed_movements
│   │   ├── requests/       # PermytClient, RequestToken, Nonce, webhook + scope views
│   │   │   ├── scopes/     # BankScopes catalogue + executors + serializers
...
- `POST /rest/<field>/<action>/` — per-scope endpoints, derived from the static catalogue (e.g. `/rest/balance/read/`, `/rest/payment/send/`)

### Dashboard API (UI-only)

- `GET /rest/profile/` — return the current user's account header (name, IBAN, balance, currency)
- `PUT /rest/profile/` — partial update (only `full_name` is editable)
- `GET /rest/movements/` — return the current user's last 20 movements
...
| `BASE_URL`               | Provider's public URL               | `http://localhost:9013`      |

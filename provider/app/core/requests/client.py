...
from .scopes.utils import NoteVaultScopes


class PermytClient(BasePermytClient):
    """Provider-side PERMYT client for NoteVault.
    ...
    def store_token(
        ...
        Validates locked scope inputs via ``NoteVaultScopes.validate_locked``,
    ...
    def get_endpoints_for_scope(self, scope: ScopeGrant) -> list[ServiceCallEndpoint]:
        ...
        return [NoteVaultScopes.get_endpoint(ref) for ref in scope]
    ...
    def process_request(
        ...
        ``NoteVaultScopes.execute``. Updates the Log row to
        ``data_served``.
        """
        user = metadata["user"]
    ...
    def process_user_connect(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        with Log.activity(
            ...
            # Seed field values for any NoteFields this user doesn't have yet
            ctx.user.seed_field_values()
    ...
    def process_user_disconnect(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        deleted outright (cascade clears LoginTokens and stored
        UserFieldValue rows). Privileged users (staff, superuser, account
        manager) keep their account: only ``permyt_user_id`` is nulled and
        login tokens are dropped so they can still sign in via the admin
        paths.
        """
        permyt_user_id = data.get("permyt_user_id")
        with Log.activity(
    ...
    def process_token_revoke(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        with Log.activity(
            ...
            if not permyt_user_id:
                raise exceptions.InvalidInputError(
                    "permyt_user_id is required for token revoke."
                )

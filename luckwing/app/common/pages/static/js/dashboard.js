/* Luckwing account-opening dashboard: live stage track + source-direct answer
   reveal, driven by Log rows the broker callback upserts as the request
   advances. Luckwing only ever sees the scoped answers — never the documents. */
...
(function () {
  ...
  const answersDoneEl = document.getElementById("answersDone");
  ...
  // ── Stage track ──────────────────────────────────────────────────
  // Requested → Awaiting your approval → Approved → Answers received
  const STAGE_ORDER = ["requested", "approval", "approved", "answers"];
  const STATUS_TO_STAGE = {
    submitted: "requested", queued: "requested", analyzing: "requested",
    awaiting: "approval",
    processing: "approved", fetching: "approved",
    answers: "answers", done: "answers", data_received: "answers",
  };
  // Stage names the client emits directly → normalized track keys.
  const STAGE_ALIAS = {
    sent: "requested", requested: "requested",
    approval: "approval", approved: "approved",
    fetching: "approved", pushing: "approved",
  ...
  function showDone() {
    if (collectedEmptyEl) collectedEmptyEl.classList.add("hidden");
    if (answersDoneEl) answersDoneEl.classList.remove("hidden");
  ...
  function applyRow(fields) {
    ...
    const rawStage = d.stage || STATUS_TO_STAGE[d.status] || null;
    const stage = rawStage ? (STAGE_ALIAS[rawStage] || rawStage) : null;
    if (stage) setStage(stage, failed);
    if (d.collected) revealCollected(d.collected);
    if (failed) {
      setLive("We couldn't finish opening your account — " + (d.reason || d.error || d.note || "please try again."), true);
  ...
  // Account opening auto-starts on connect; reflect whatever state already
  // exists (covers a reload mid-flow or after completion), then live WS takes
  // over.

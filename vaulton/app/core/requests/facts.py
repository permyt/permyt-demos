...
# Which authority answers each scope reference — used to group displayed facts
# by source so the dashboard reads like separate real parties answered.
SOURCE_BY_REFERENCE = {
...
def collect_facts(responses: list[Any]) -> list[dict]:
    """Turn ``call_services`` output (a list of ``{reference: payload}`` dicts)
    into ``[{source, label, value}]`` for the dashboard's provenance reveal."""
    facts: list[dict] = []

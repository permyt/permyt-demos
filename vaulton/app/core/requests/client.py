...
# trader's connected providers (identity service, bank, sanctions screening).
ONBOARDING_DESCRIPTION = (
    "Verify the customer's identity, confirm their residential address, confirm "
    "the source of their £40,000 deposit, and confirm they are not on any "
    "sanctions or PEP list — to approve the account for trading."
)
...
class PermytClient(BasePermytClient):
    ...
    def process_user_connect(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        with Log.activity(
            ...
            # trader doesn't have to click anything — by the time the dashboard
    ...
    def process_request_status(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if status == "completed":
            ...
            # (the first spends time fetching from providers), so a plain
            # answers-present check isn't enough: claim the request atomically
            # under a row lock on a generic in-progress flag.
            with transaction.atomic():
    ...
    def _handle_completion(self, record, services: list[Any]) -> None:
        """Approved → pull the scoped answers from the authoritative providers and
        surface them (grouped by source) on the dashboard. Each step is recorded
        on the same Log row so the dashboard advances its stage track live."""
        if not services:
            record(
                action="ended",
        ...
        # Stage: fetching answers from the authoritative sources (still within
        # the "Approved" stage of the track until the answers land).
        record(
            action="fetching",
            data={"status": "fetching", "stage": "approved", "num_services": len(services)},
        )
        try:
        ...
        # Stage: answers received — surface them (with provenance) for the reveal.
        record(
            action="answers",

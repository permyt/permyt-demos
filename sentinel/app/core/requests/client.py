...
from .scopes.utils import SentinelScopes


class PermytClient(BasePermytClient):
    """Provider-side PERMYT client for the Sentinel Screening provider.
    ...
    def store_token(
        ...
        Validates locked scope inputs via ``SentinelScopes.validate_locked``,
    ...
    def get_endpoints_for_scope(self, scope: ScopeGrant) -> list[ServiceCallEndpoint]:
        ...
        return [SentinelScopes.get_endpoint(ref) for ref in scope]
    ...
    def process_request(
        ...
        ``SentinelScopes.execute``. Updates the Log row to
        ``data_served``.
        """
        user = metadata["user"]
    ...
    def process_user_connect(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        the login token, creates or links the local screening record via
        ``permyt_user_id``, seeds it (clear), and marks the session as
        authenticated.
        """
        ...
        with Log.activity(
            ...
            # Ensure the screening record exists (outcomes default to clear).
            ctx.user.seed()
    ...
    def process_user_disconnect(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        deleted outright (cascade clears LoginTokens and the screening
        record). Privileged users (staff, superuser, account manager)

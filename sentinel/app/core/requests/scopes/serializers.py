...
class ScopeSerializer(serializers.Serializer):  # pylint: disable=abstract-method
    ...
    * ``locked_data=<dict>`` — full validation, then enforce that every
    ...
    Sentinel's scopes declare no inputs, but the base class is kept so the
    catalogue, ``SentinelScopes`` validation surface, and any future
    input-bearing scope continue to work unchanged.
    """

...
class SentinelScopes:
    """Static scope catalogue for the Sentinel Screening provider.

    Backed by ``catalogue.SCOPES`` — a tuple of ``ScopeDescriptor`` records.
    Each scope reference is ``<slug>.<action>`` where action is ``check`` for
    a screening answer (``read`` is also accepted for parity).
    """

    @staticmethod
    def _parse_reference(reference: str) -> tuple[str, str]:
        """Split a scope reference like ``sanctions.check`` into ``(slug, action)``."""
    ...
    def get_input_fields(self, reference: str) -> dict[str, str]:
        """Return the input field definitions for a scope (empty dict for checks)."""
    ...
    def validate_params(self, reference: str, params: dict, locked: dict | None = None) -> dict:
        ...
        Sentinel's scopes declare no inputs, so this is effectively a no-op
        that returns the (empty) params. The locked-input enforcement path is
        kept for any future input-bearing scope.
        """
    ...
    def execute(self, user, reference: str, params: dict) -> dict:
        """Run the scope's executor against the user's screening record."""
        desc = self._get_descriptor(reference)

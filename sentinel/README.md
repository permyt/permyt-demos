# Sentinel Screening — PERMYT Demo Provider

A working Django app (port 9018) that demonstrates how to build a **compliance-grade watchlist Provider** using the [`permyt`](https://github.com/LeopardLabs/permyt-api-python) Python SDK.
...
Sentinel Screening implements the **Provider** and **Connect** roles for PERMYT access requests. It is a compliance-grade watchlist authority: each connected subject has a screening record of four booleans (all seeded **clear** on connect, editable from the dashboard so denials can be demoed).

It exposes four boolean **check** scopes (no inputs, all high-sensitivity, `prompt_once` consent):
...
Each check leaks a single bit — **the authoritative source answers directly** (source-direct provenance), without exposing any underlying list entries or records.
...
`SCOPES` is a tuple of `ScopeDescriptor` records. Each entry maps a scope reference (e.g. `sanctions.check`) to a name, description, optional input serializer, and an executor callable. `SentinelScopes` (in `scopes/utils.py`) implements the same public surface the SDK expects — `validate_params`, `validate_locked`, `execute`, `get_endpoint` — but reads from the static catalogue instead of the database.

To add a new scope, append one descriptor to `SCOPES` and re-run `python manage.py update_scope`.

### Scope inputs

Sentinel's four checks declare **no inputs** — each returns a single boolean answered directly from the register. The `ScopeSerializer` base class and the broker-locked input enforcement in `SentinelScopes.validate_params` / `validate_locked` are retained so any future input-bearing scope works unchanged.
...
The Connect flow is a general-purpose mechanism for linking a user's PERMYT identity to an action in the provider. The pattern: generate a connect token with `client.generate_connect_token()`, store a record of what that token is for, and render the QR payload. When the user scans the code, the broker calls back to the inbound webhook with the user's identity. `process_user_connect()` then authenticates the user and calls `seed()` to ensure the screening record exists — the four outcomes default to the **clear** state, so a freshly connected subject passes screening unless flagged.

When the user revokes the connection from their PERMYT app, the broker fires a mirror `user_disconnect` callback. `process_user_disconnect()` is idempotent. Regular users have no non-PERMYT login path, so the `User` row is deleted outright — cascade wipes the screening record and login tokens. Privileged users (staff / superuser / account manager) keep their account; only `permyt_user_id` is nulled and the login tokens are dropped so they can still reach the admin paths.
...
python manage.py runserver 9018
```
...
- **Name**: "Sentinel Screening"
- **Callback URL**: `http://localhost:9018/rest/permyt/inbound` (production: `https://sentinel.permyt.io/rest/permyt/inbound`)
...
│   ├── core/
│   │   ├── users/          # User (screening record), LoginToken, QR login
│   │   ├── requests/       # PermytClient, RequestToken, Nonce, webhook + scope views
│   │   │   ├── scopes/     # SentinelScopes catalogue (four boolean checks)
...
| `BASE_URL`               | Provider's public URL               | `http://localhost:9018`      |

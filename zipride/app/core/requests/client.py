...
# driver's connected authoritative sources (national identity service:
# identity, right to work, driving licence).
ONBOARDING_DESCRIPTION = (
    "Verify the applicant's identity, confirm they have the right to work, and "
    "confirm they hold a valid driving licence with no disqualifications — to "
    "approve them as a driver."
)
...
class PermytClient(BasePermytClient):
    ...
    def process_user_connect(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        with Log.activity(
            ...
            # Kick off the driver-verification request immediately on connect, so
            # the driver doesn't have to click anything — by the time the
            # dashboard loads, the request is already in flight and the stage
            # track is live.
            self.start_onboarding(ctx.user)
            return {"logged": True}

    def start_onboarding(self, user, description: str | None = None) -> str | None:
        """Submit the driver-verification access request for ``user`` and log it
        for the dashboard. Returns the broker request id (or ``None`` on failure).
    ...
    def process_request_status(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if status == "completed":
            ...
            # (the first spends a few seconds fetching), so a plain
            # ``collected`` check isn't enough: claim the request atomically
            # under a row lock.
            with transaction.atomic():
    ...
    def _handle_completion(self, record, services: list[Any]) -> None:
        """Approved → pull the scoped answers from the authoritative sources and
        surface them directly. Each step is recorded on the same Log row so the
        onboarding dashboard advances its stage track live."""
        if not services:
            record(
                action="ended",
